...
def generate_rss_feed(feed_number, entries):
    rss_feed = ET.Element("rss", version="2.0")
    channel = ET.SubElement(rss_feed, "channel")
    ET.SubElement(channel, "title").text = f"Paul Graham Essays - Feed {feed_number}"
    ET.SubElement(channel, "link").text = "https://paulgraham.com/"
    ET.SubElement(channel, "description").text = f"A collection of essays by Paul Graham - Feed {feed_number}"

    message = f"Starting to generate RSS feed {feed_number}..."
    print(message)
    logging.info(message)
    ...
    message = f"Finished generating RSS feed {feed_number}."
    print(message)
    logging.info(message)
    
    # Save RSS feed to a file
    tree = ET.ElementTree(rss_feed)
    tree.write(f"rss_feed_{feed_number}.xml", encoding="utf-8", xml_declaration=True)
    message = f"RSS feed {feed_number} saved to rss_feed_{feed_number}.xml."
    print(message)
    logging.info(message)

def generate_rss_feeds():
    batch_size = 5
    for i in range(0, len(toc), batch_size):
        entries = toc[i:i + batch_size]
        if entries:
            generate_rss_feed(i // batch_size + 1, entries)
...
if __name__ == '__main__':
    ...
    message = f"Serving rss_feed_1.xml on http://0.0.0.0:{PORT}"
    print(message)
    logging.info(message)
